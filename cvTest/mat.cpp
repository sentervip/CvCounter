...
unsigned char pixel[HEIGHT][WIDTH];  //记录每个像素点的灰度值
...
int g_begin[2];   //记录开始数周长的初始点坐标(i,j)
...
int main3( )
{
    ...
    IplImage* image = cvLoadImage("/Users/HFY/Downloads/redcell/test/cell.bmp",0) ;   // 以灰度方式打开图像
    if(!image) return -1;
...
void DoPerimeter( int i,int j,int b,int tmp)
{
    ...
    {
        case 0:
            if(i!=g_begin[0] || j-1!=g_begin[1])    // 判断是否与起始点重合
            {
                perimeter[tmp]++;
